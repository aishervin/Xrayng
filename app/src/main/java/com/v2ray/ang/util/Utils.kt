...
object Utils {
    ...
    private fun tryDecodeBase64(text: String?): String? {
        ...
        try {
            return java.util.Base64.getDecoder().decode(text).toString(Charsets.UTF_8)
        } catch (e: Throwable) {
            // Fallback for JVM URL-safe
        }
        try {
            return java.util.Base64.getUrlDecoder().decode(text).toString(Charsets.UTF_8)
        } catch (e: Throwable) {
            // Failed
        }
        return null
