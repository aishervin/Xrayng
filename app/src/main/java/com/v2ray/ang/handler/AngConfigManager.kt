...
object AngConfigManager {
    ...
    private val configFmtParsers: Map<String, (String) -> ProfileItem?> by lazy {
        mapOf(
            ...
            AppConfig.TG_SOCKS to SocksFmt::parse,
            "https://t.me/socks" to SocksFmt::parse,
            ...
            AppConfig.TG_HTTP to HttpFmt::parse,
            "tg://proxy" to HttpFmt::parse,
            "https://t.me/http" to HttpFmt::parse,
            "https://t.me/proxy" to HttpFmt::parse,
        )
    }
        ...
        try {
            ...
            servers.lines()
                ...
                .forEach { str ->
                    ...
                    if (trimmed.isNotEmpty() && parseConfig(trimmed, "", null) == null && Utils.isValidSubUrl(trimmed)) {
                        count += importUrlAsSubscription(trimmed)
                    }
                }
            return count
        } catch (e: Exception) {
    ...
    private fun parseConfig(
        ...
        try {
            ...
            val config = configFmtParsers.firstNotNullOfOrNull { (scheme, parser) ->
                if (trimmed.startsWith(scheme, ignoreCase = true)) parser(trimmed) else null
            } ?: parseRawProxy(trimmed)
    ...
    /**
     * Parses raw proxy line formats such as:
     * host:port, host:port:user:pass, user:pass@host:port, socks5:host:port:user:pass, http:host:port
     */
    private fun parseRawProxy(str: String): ProfileItem? {
        val trimmed = str.trim()
        if (trimmed.isEmpty()) return null

        if (trimmed.startsWith("socks5:", ignoreCase = true) ||
            trimmed.startsWith("socks:", ignoreCase = true) ||
            trimmed.startsWith("socks4:", ignoreCase = true)
        ) {
            val content = trimmed.substringAfter(":")
            return SocksFmt.parse("socks://$content")
        }
        if (trimmed.startsWith("http:", ignoreCase = true) ||
            trimmed.startsWith("https:", ignoreCase = true)
        ) {
            val content = trimmed.substringAfter(":")
            return HttpFmt.parse("http://$content")
        }
        ...
        if (hashIdx != -1) {
            remarks = line.substring(hashIdx + 1).trim()
            line = line.substring(0, hashIdx).trim()
        }

        if (line.contains("@")) {
            val socksAttempt = SocksFmt.parse("socks://$line" + if (remarks.isNotEmpty()) "#$remarks" else "")
            if (socksAttempt != null) return socksAttempt
            val httpAttempt = HttpFmt.parse("http://$line" + if (remarks.isNotEmpty()) "#$remarks" else "")
            if (httpAttempt != null) return httpAttempt
        } else if (line.contains(":")) {
            val parts = line.split(":")
            if (parts.size == 2 || parts.size == 4) {
                val port = if (parts.size == 2) parts[1].toIntOrNull() else parts[1].toIntOrNull() ?: parts[3].toIntOrNull()
                if (port != null && port in 1..65535) {
                    val isHttpPort = port in listOf(80, 8080, 8888, 3128, 8000, 8081, 8443)
                    val parsed = if (isHttpPort) {
                        HttpFmt.parse("http://$line" + if (remarks.isNotEmpty()) "#$remarks" else "")
                    } else {
                        SocksFmt.parse("socks://$line" + if (remarks.isNotEmpty()) "#$remarks" else "")
                    }
